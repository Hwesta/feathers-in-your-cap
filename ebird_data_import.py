    ...
    with open(file_path, 'r') as f:
        ...
        for e in reader:
            try:    
                ...
                obs = Observation(observation_id=observation_id, number_observed=number_observed, is_x=is_x, age_sex=age_sex, species_comments=species_comments, species=sp, subspecies=subsp, breeding_atlas_code=atlas, date_last_edit=last_edit, has_media=has_media, bcr_code=bcr, usfws_code=usfws, checklist=check)
                obs.save()
...
def parse_start_duration(checklist_date, checklist_time, checklist_duration):
    ...
    if checklist_duration == '':
        ...
        duration_start = datetime(1900, 1, 1, 0, 0, 0, 0)
        h = duration_mins // 60
        m = duration_mins % 60
        duration_end = datetime(1900, 1, 1, h, m, 0, 0)
